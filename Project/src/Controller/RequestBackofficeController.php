...
class RequestBackofficeController extends AbstractController
{
    ...
    public function postAddTicketing(TicketingRepository $ticketingRepository, ImageTicketingRepository $imageTicketingRepository, PartnershipRepository $partnershipRepository, SubscriberRepository $subRep, Validator $validate, MailerInterface $mailer, Request $request): Response
    {
        try {
            ...
            if (
                ...
                // slug de l'image
                $image1->setName($ticketing->getId() . '-' . str_replace(' ', '-', html_entity_decode($ticketing->getName())) . '-image-1' . '.' . $image->guessExtension());
                $image1->setNumero(1);
                ...
                if (isset($request->files->get('ticketing')['image2'])) {
                    ...
                    // slug de l'image
                    $image2->setName($ticketing->getId() . '-' . str_replace(' ', '-', html_entity_decode($ticketing->getName())) . '-image-2' . '.' . $image->guessExtension());
                    $image2->setNumero(2);
                ...
                if (isset($request->files->get('ticketing')['image3'])) {
                    ...
                    // slug de l'image
                    $image3->setName($ticketing->getId() . '-' . str_replace(' ', '-', html_entity_decode($ticketing->getName())) . '-image-3' . '.' . $image->guessExtension());
                    $image3->setNumero(3);
                ...
                if (isset($request->files->get('ticketing')['image4'])) {
                    ...
                    // slug de l'image
                    $image4->setName($ticketing->getId() . '-' . str_replace(' ', '-', html_entity_decode($ticketing->getName())) . '-image-4' . '.' . $image->guessExtension());
                    $image4->setNumero(4);
        ...
        try {
            ...
            if ($validate->checkInputString($firstname) && $validate->checkInputString($lastname) && $validate->checkInputString($function)) {
                ...
                if (isset($request->files->get('member')['profil'])) {
                    ...
                    // slug de l'image
                    $member->setProfil($member->getId() . '-' . str_replace(' ', '-', html_entity_decode($member->getFirstName())) . '-' . str_replace(' ', '-', html_entity_decode($member->getLastName())) . '.' . $image->guessExtension());
    ...
    #[Route(path: 'modif-partenaire', name: 'post-edit-partnership', methods: ['POST'])]
        ...
        try {
            ...
            if ($validate->checkInputString($name) && $validate->checkInputString($description) && $validate->checkInputString($request->get('partnership')['link'])) {
                ...
                if (isset($request->files->get('partnership')['image'])) {
                    ...
                    // slug de l'image
                    $partner->setImage($partner->getId() . '-' . str_replace(' ', '-', html_entity_decode($partner->getName())) . '.' . $image->guessExtension());
                    // deplacer l'image dans le fichier 
    ...
    #[Route(path: 'ajout-partenaire', name: 'post-add-partnership', methods: ['POST'])]
